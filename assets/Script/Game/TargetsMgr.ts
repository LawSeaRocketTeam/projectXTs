...
var TargetIdx = 1;
@ccclass
export default class TargetsMgr extends BaseComponent {

    @property(cc.Prefab)
    targetPrefab : cc.Prefab = null;
    @property(cc.Prefab)
    fortPrefab : cc.Prefab = null;

    public cached_targets : cc.NodePool = null;
    public use_targets : any[] = [];
    public beKilled_targetsId : any[] = [];
    ...
    //从正在使用数组中移除目标
    private  _removeTargetInUse(_target:any){
    ...
    public addIdleTarget(_target:any){
    ...
    //检测某点是否在各个目标内
    //是则返回目标
    public checkPointIsInTargets(_point:cc.Vec2){
        this.use_targets.forEach(function(item, index, arr) {
    ...
    public checkTargetsBeShoot(_shootPoint:cc.Vec2){
        let ret = false
        let perfect = false
        let reactionTime = 0;
        this.use_targets.forEach(function(item, index, arr) {
    ...
    //检查场上是否有同类型怪
    public checkIsExistSameTypeTarget(_id:number){
    ...
    //返回场上的目标个数
    public getInUseTargetsCount(){
        return this.use_targets.length;
    }

    //统计被击杀的目标
    public addBeKillId(_id:number){
        let bFind = false;
    ...
    //获取指定目标被击杀的个数
    public getBeKillCountById(_id:number){
        for(let i in this.beKilled_targetsId){
            let v = this.beKilled_targetsId[i];
            if(v.id == _id){
                return v.count;
    ...
    //创建一个堡垒目标
    public getFortTarget (_fortId:number){
